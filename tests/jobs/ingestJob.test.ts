...
describe('runIngestJob', () => {
  it('collects, persists, normalizes, compiles, and exports a mixed-source workspace', async () => {
    const root = await makeTempDir('waybook-workspace-');
    ...
    await writeFile(path.join(repoDir, 'README.md'), '# waybook\n');
    await execFileAsync('git', ['add', 'README.md'], { cwd: repoDir });
    await execFileAsync('git', ['commit', '-m', 'feat: add workspace readme'], { cwd: repoDir });

    await mkdir(path.join(experimentsRoot, 'run-a'), { recursive: true });
    await writeFile(
      path.join(experimentsRoot, 'run-a', 'metrics.json'),
      JSON.stringify({ accuracy: 0.91 }, null, 2)
    );
    await writeFile(path.join(experimentsRoot, 'run-a', 'summary.md'), '# Run A\nReady.\n');
    ...
    memDb.prepare(
      ...
      'waybook',
      'Implement M1',
      'Need a real persisted pipeline.',
      'Created the mixed collector design.',
    ...
    const ingestResult = await runIngestJob(config);
    const db = createDatabaseClient(config.databasePath);
    const events = await listResearchEvents(db);
    const entities = await listWikiEntities(db);
    const exportResult = await runExportJob(config, db);

    expect(ingestResult.rawEventsInserted).toBeGreaterThan(0);
    expect(events.length).toBeGreaterThan(0);
    expect(entities.length).toBeGreaterThan(0);
    expect(exportResult.filesWritten.length).toBeGreaterThan(0);
  });
});
